...
(function($) {
    ...
    $.ligerDefaults.ComboBox = {
        ...
        showRoot:true,          //不显示清空按钮
        rootId: '0',             //根节点ID,默认为0
        rootText: '根节点'        //根节点显示的值
    };
    ...
    $.fn.ligerComboBox = function(options) {
        this.each(function() {
            ...
            var g = {
                ...
                setGrid: function(grid) {
                    ...
                    grid.onAfterShowData = function(){
                        if (!g.gridManager) return;

                        if(!p.initValue) return false;
                        //设置表格组件的默认值
                        var splitValue = p.initValue.split(p.split);
                        for(var i in splitValue)
                            g.gridManager.selectRow(grid.pkName, splitValue[i]);

                        //设置下拉框的默认值
                        var rows = p.grid.checkbox ? g.gridManager.getCheckedRows() : [g.gridManager.getSelectedRow()];

                        var value = [], text = [];
                        for(var j in rows){
                            value.push(rows[j][p.valueField]);
                            text.push(rows[j][p.textField]);
                        }
                        po.changeValue(value.join(p.split), text.join(p.split));
                    }
                    grid.width = "100%";
            ...
            if (this.tagName.toLowerCase() == "input") {
                ...
                g.inputText[0].id = g.inputText[0].name = g.inputText.attr('id') + '_txt';
            ...
            //下拉框
            g.selectBox = $('<div class="l-box-select"><div><table width="100%" cellpadding="0" cellspacing="0" border="0" class="l-box-select-table"></table></div></div>');
            ...
            //清空按钮初始化
            if (p.showRoot) {
                g.clear = g.wrapper.append('<div class="l-text-clear"><a>' + p.rootText + '</a></div>').find(".l-text-clear");
            }
            ...
            if (p.showRoot) {
                g.clear.click(function() {
                    if(g.treeManager){
                        //1.树形结构时
                        var tree = g.treeManager.tree;
                        //多选的情况
                        $('.l-checkbox-checked', tree).each(function(){
                            $(this).removeClass('l-checkbox-checked').addClass('l-checkbox-unchecked');
                        });
                        //单选的情况
                        $('.l-selected', tree).each(function(){
                            $(this).removeClass('l-selected');
                        });
                    } else if(g.gridManager){
                        //2.表格
                        var grids = g.gridManager.getCheckedRowObjs();
                        for(var i in grids){
                            $(grids[i]).removeClass('l-checked');
                        }

                        //全选按钮
                        var gridbody = g.gridManager.gridbody;
                        $('.l-grid-hd-cell-checkbox', gridbody.prev()).parent('tr').removeClass('l-checked');
                    } else {
                        //3.其他情况
                        $('.l-selected', g.selectBox).each(function(){
                            $(this).removeClass('l-selected');
                        });
                    }

                    po.changeValue(p.rootId, "");
                });
            }
            g.resizing = false;
