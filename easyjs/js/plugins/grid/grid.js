...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        onError:null, //错误事件
        onSubmit:null, //提交前事件
        dateFormat:'hh:mm yyyy-MM-dd', //默认时间显示格式
    ...
    $.ligerAddGrid = function (grid, p) {
        ...
        gridhtmlarr.push("                </div>");
        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input id='curPage' type='text' size='4' value='1' style='width:20px' maxlength='3'/> / <span></span></span></div>");
        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        var g = {
            ...
            getRowByKeyValue:function (key, value) {
                if(!key || !value)
                    return null;

                for (var p in g.records) {
                    if (g.records[p][key] == value)
                        return $("tbody:first > .l-grid-row[rowid=" + p + "]", g.gridbody).get(0);
                }
            ...
            getClickRow:function (currentRow) {
                var rowid, rowdata;
                var row = $(currentRow);
                //判断点击的是哪个div或者span，然后得到rowid
                if (currentRow.tagName.toLowerCase() == 'td') {
                    rowid = row.parent().attr("rowid");
                } else if (currentRow.tagName.toLowerCase() == "div" &&
                    ($(currentRow).hasClass("l-grid-tree-space") || $(currentRow).hasClass("l-grid-sub-arrow") )) {
                    rowid = row.parent().parent().parent().parent().attr("rowid");
                } else if (currentRow.tagName.toLowerCase() == "div") {
                    rowid = row.parent().parent().attr("rowid");
                } else if (currentRow.tagName.toLowerCase() == 'img') {
                    //比较特殊的情况，树分支前有图标的需要特殊处理
                    rowid = row.parent().parent().parent().parent().parent().attr("rowid");
                } else {
                    rowid = row.parent().parent().parent().attr("rowid");
                }
                rowdata = g.getRow(rowid);
                return rowdata;
            },
            /**
             * 选中表格中的一行
             * @param pkValue   主键值
             */
            selectRow: function(pkName, pkValue){
                var rowdatas = g.getRowByKeyValue(pkName, pkValue);

                $(rowdatas).addClass(p.checkbox ? 'l-checked' : 'l-selected');
            }
        };
        var po = {
            init:function () {
            ...
            initEven:function () {
                ...
                //表头 - 经过和点击事件
                $(".l-grid-hd-cell", g.gridheader).hover(
                    function () {
                    },
                    function () {
                ...
                $(grid).dblclick(function (e) {
                    var obj = (e.target || e.srcElement);
                    var objParent;
                    /*if($(obj).hasClass("l-grid-tree-content")){
                     objParent = $(obj).parent();
                     } else {
                     objParent = $(obj);
                     }*/
                    var tagName = $(obj).parent()[0].tagName.toLowerCase();
                    if (tagName == 'div') {
                        objParent = $(obj).parent();
                    ...
                    if (obj.tagName.toLowerCase() == "span" &&
                        (objParent.hasClass("l-grid-row-cell-inner") ||
                            objParent.hasClass("l-grid-row-cell-inner-fixedheight"))) {
                        if (p.enabledEdit && !p.dblClickToEdit) {
                            var row = null;
                            if (objParent.hasClass("l-grid-row-cell")) row = objParent.parent();
                            else row = objParent.parent().parent();
                            //第一次选择的时候不允许编辑，第二次才允许
                            if (p.allowUnSelectRow || row.hasClass("l-selected-again"))
                                po.applyEditor(objParent);
                        } else {
                            if (p.dblClickRow) {
                                //点击了非span的空白处，执行dblClickRow函数
                                return p.dblClickRow({data:g.getClickRow(obj)}, e);
                            }
                        }
                    } else {
                        if (p.dblClickRow) {
                ...
                //添加事件列表分页跳转
                $("#curPage").bind("blur", function () {
                    var obj = $(this);
                    if (!/^[1-9]\d*$/.test(obj.val())) {
                        top.dialog.warn("请输入正整数!", "警告", function () {
                            obj.value = "";
                            obj.focus();
                        });
                    } else {
            ...
            showData:function (data) {
                ...
                else if (p.tree)//启用分页模式
                {
                    ...
                    if (!p.tree.isParent) p.tree.isParent = function (rowData) {
                        //修改如果子节点包含空的children则默认为没有children
                        var exist = 'children' in rowData && rowData.children != "";
                        return exist;
                    };
                    ...
                    $(g.headers).each(function (headerCellIndex, headerInfor) {
                        ...
                        if (name.indexOf(".") > 0) {
                            while (name.indexOf(".") > 0) {
                                str = name.substr(0, name.indexOf("."));
                                name = name.substr(name.indexOf(".") + 1);
                                if (jsonName[str] && name.indexOf(".") == -1) {
                                    jsonName = jsonName[str][name];
                                } else
                                    jsonName = jsonName[str];
                            }
                        } else {
                            jsonName = jsonName[this.columnname];
                        }

                        var content = "";
                        //初始化数据的方法
                        if (column.initDataFunc) {
                            content = column.initDataFunc(jsonName, item);
                        } else {
                            content = po.getCellContent(item, i, jsonName, column, tree, level);
                        }

                        if (column && column.dataCategory && column.dataCategory == 'switch') {
                            var contextPath = easyloader.URI;
                            if (content == 1) {//开关开启
                                content = "<img src='" + contextPath + "/skins/default/images/btn_qiyong.gif' />";
                            } else if (content == 0) {
                                content = "<img src='" + contextPath + "/skins/default/images/btn_jinyong.gif' / >";
                            }
                        }

                        if (column && column.dataCategory && column.dataCategory == 'date') {
                            if (content != "") {
                                var date = new Date();
                                date.setTime(content);
                                var format = column.format ? column.format : p.dateFormat;
                                content = g.getFormatDate(date, format)
                            }
                        }

                        //可以为某一列设置默认值(当给定值为空时，显示默认值)
                        if (column && column.dataCategory && column.dataCategory == 'defaultValue') {
                            if (content == '') {
                                content = column.defaultValue;
                            }
                        }

                        //显示序号
                        if (column && column.dataCategory && column.dataCategory == 'index') {
                            content = index++;
                        }

                        //文件大小换算
                        if (column && column.dataCategory && column.dataCategory == 'fileSize') {
                            var size = content * 1.00;
                            if (size < 1024) {
                                content = size + "b";
                            } else if (size >= 1024 && size < 1024 * 1024) {
                                content = po.toDecimal(size / (1024)) + "k";
                            } else if (size >= 1024 * 1024 && size < 1024 * 1024 * 1024) {
                                content = po.toDecimal(size / (1024 * 1024)) + "M";
                            } else if (size >= 1024 * 1024 * 1024 && size < 1024 * 1024 * 1024 * 1024) {
                                content = po.toDecimal(size / (1024 * 1024 * 1024)) + "G";
                            } else if (size >= 1024 * 1024 * 1024 * 1024 && size < 1024 * 1024 * 1024 * 1024 * 1024) {
                                content = po.toDecimal(size / (1024 * 1024 * 1024 * 1024)) + "T";
                            }
                        }
                        //
                        if (column && column.dataCategory && column.dataCategory == 'fileOperate') {
                            var o = jsonName;
                            content = "<span><a class='small_button' href='" + easyloader.URI + "/attach/" + o[0] + o[1] + "' target='_blank'>查看</a></span>";
                        }
            ...
            getTreeCellHtml:function (tree, oldContent, rowData, level) {
                ...
                if (isExtend && isParent)
                    ...
                    content += "<div class='l-grid-tree-space'></div><span class='l-grid-sub-arrow'><img src='" + easyloader.URI + "/skins/default/images/icon_arrow.gif'>&nbsp;</span>";
            ...
            setRowEven:function (rowobj) {
                $(rowobj).bind("contextmenu", function (e) {
                    ...
                    if (p.onRClickToSelect)
                        if (!$(this).hasClass("l-checked")) {
                            $(this).addClass("l-checked").siblings(".l-checked").removeClass("l-checked");
                        }
                    if (p.onContextmenu) {
                ...
                $(rowobj).hover(
                    ...
                    function (e) {
                        if (p.checkbox) {
                            ...
                            if (!($(srcObj).hasClass("l-grid-tree-link-close") || $(srcObj).hasClass("l-grid-tree-link-open")) && !selectRowButtonOnly || $(srcObj).hasClass("l-grid-row-cell-btn-checkbox")) {
                                var row = $(this);
                                var index = row.attr('rowindex');
                                var rowid = row.attr("rowid");
                                var oncheck = row.hasClass("l-checked");
                                if (p.onBeforeCheckRow) {
                                    if (p.onBeforeCheckRow(!oncheck, g.getRow(rowid), index, row[0]) == false) return false;
                                }
                                if (e.ctrlKey || $(srcObj).hasClass("l-grid-row-cell-btn-checkbox")) {
                                    if (oncheck) {
                                        row.removeClass("l-checked");
                                        $("tbody :first").removeClass("l-checked");
                                    }
                                    else
                                        row.addClass("l-checked");
                                } else {
                                    if (oncheck && $(".l-checked", row.parent()).size() == 1)
                                        row.removeClass("l-checked");
                                    else {
                                        row.addClass("l-checked").siblings().removeClass("l-checked");
                                        $("tbody :first").removeClass("l-checked");
                                    }
                                }
                                p.onCheckRow && p.onCheckRow(!oncheck, g.getRow(rowid), index, row[0]);
                            }
                            if (!p.enabledEdit)
                                return;
                        }
                        ...
                        var rowid = $(this).attr('rowid');
                        var row = $(this);
                        if (!($(srcObj).hasClass("l-grid-tree-link-close") || $(srcObj).hasClass("l-grid-tree-link-open"))) {
                            row.addClass("l-checked").siblings().removeClass("l-checked");
                        }
                        if (p.onDblClickRow) {
    ...
    //获取当前表格json数据
    $.getGridData = function (gridId, selRow) {
        var manager = $("#" + gridId).ligerGetGridManager();
        var data = selRow ? manager.getSelectedRow() : manager.getData();
        if (selRow && !data)return '请选择行！';
        return JSON.stringify(data);
    };

    //获取复选框选中的行的json数据
    $.getCheckedGridData = function (gridId) {
        var manager = $("#" + gridId).ligerGetGridManager();
        if (!manager)return;
        var checkedData = manager.getCheckedRows();
        return JSON.stringify(checkedData);
    }

    //保存当前表格数据
    $.saveGridData = function (gridId, url, execFunc) {
        var manager = $("#" + gridId).ligerGetGridManager();
        var data = manager.getData();
        $.ajax({
            type:'post',
            url:url ? url : '',
            data:{'grid_data':JSON.stringify(data)},
            success:function (ret) {
                ...
                $.ligerDialog.error(stauts);
            }
        });
    };

    //删除选中行数据
    $.deleteSeletedData = function (gridId, url, execFunc) {
        var manager = $("#" + gridId).ligerGetGridManager();
        if (!manager)return;
        //先判断复选框是否有选中
        var checkedData = manager.getCheckedRows();
        ...
        if (checkedData == null || checkedData.length == 0) {
            ...
            if (checkedData != null)
                ids.push(checkedData.pkId);
        } else {
            for (var i = 0; i < checkedData.length; i++)
                ids.push(checkedData[i].pkId);
        }
        ...
        if (ids == "") {
            top.$.ligerDialog.alert("请选择要删除的行！", "提示", "warn");
            return;
        }
        top.$.ligerDialog.confirm("确定要删除选定的行吗？", function (r) {
            if (r) {
                $.ajax({
                    type:'post',
                    url:url ? url : '',
