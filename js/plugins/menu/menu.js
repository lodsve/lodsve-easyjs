    ...
    $.ligerMenu = function (p) {
        ...
        var g = {
            show:function (options, menu) {
                ...
                var win_height = $(window).height() + $(document).scrollTop();
                if (menu == undefined) menu = g.menu;
                if (options && options.left != undefined) {
                    var _left = (win_width - options.left > p.width) ? options.left : (options.left - p.width);
                    menu.css({ left:_left });
                }
                if (options && options.top != undefined) {
                    var _height = (win_height - options.top > menu.height()) ? options.top : (win_height - menu.height() - 10);
                    menu.css({ top:_height });
                }
                //单选按钮变灰
                if ($(".l-checked").size() > 1) {
                    $(".l-menu-item[singlerow=true]").addClass("l-menu-item-disable");
                } else {
                    $(".l-menu-item").removeClass("l-menu-item-disable");
                }
                menu.show();
                g.updateShadow(menu);
            },
            ...
            addItem:function (item, menu) {
                ...
                item.singleRow && ditem.attr("singleRow", item.singleRow);
                if (item.children) {
